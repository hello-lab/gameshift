...
export default function TeamLeaderboardPage() {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        const response = await fetch("/api/team/leaderboard", { cache: "no-store" });
        if (!response.ok) {
        ...
        setError("Failed to load team leaderboard. Please try again later.");
  ...
  return (
    <div className="relative min-h-screen px-6 py-16 text-white font-display bg-verse-bg">
      ...
      <header className="relative z-10 flex items-center justify-between max-w-6xl mx-auto mb-12">
        <div>
          <p className="text-purple-200 text-xs tracking-[0.3em] uppercase font-pixel">Squad Rankings</p>
      ...
      <main className="relative z-10 max-w-6xl mx-auto">
        {loading ? (
          <div className="glass-card p-12 text-center">
            <div className="text-xl text-purple-200">Loading team leaderboard...</div>
          </div>
        ) : error ? (
          <div className="glass-card p-12 text-center">
          ...
          <div className="glass-card p-12 text-center">
            <div className="text-purple-200">No teams yet. Create one to compete!</div>
          </div>
        ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="col-span-4">
                      <p className="font-pixel text-white">{entry.name}</p>
                      <p className="text-xs text-purple-300/60">
                        Created {new Date(entry.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                    <div className="col-span-3">
                      <p className="text-sm text-purple-200">{entry.leaderName}</p>
                    </div>
                    <div className="col-span-1 text-center">
                      <p className="text-sm text-purple-200">{entry.memberCount}</p>
                    </div>
